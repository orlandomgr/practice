...
def dfs(node, distance, level, topNodes):
    if not node:
        return

    if distance not in topNodes or topNodes[distance][1] > level:
        topNodes[distance] = (str(node.info), level)

    dfs(node.left, distance - 1, level + 1, topNodes)
    dfs(node.right, distance + 1, level + 1, topNodes)
...
def topView(root):
    ...
    for key in sorted(topNodes):
        result.append(topNodes[key][0])
