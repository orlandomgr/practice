from typing import List
from myUtils.Utils import printResult
from collections import defaultdict
import math
from collections import deque
import heapq
from myUtils.Utils import Node

class Solution:
    def parse(self, input: List[str]):
        ll = {}
        edges = set()
        nodes = set()
        for i in input:
            parts = i.split(":")
            source = parts[0]
            dest = parts[1].lstrip().split(" ")
            filtered = []
            for d in dest:
                filtered.append(d)
                edges.add((source,d))
                nodes.add(source)
                nodes.add(d)
            graph[source] = filtered
        return graph, nodes, edges

    def dfs(self, graph, node, path):
        q = deque()
        q.append((node, path))

        while q:
            node, path = q.popleft()
            path.append(node)
            pathStr = "_".join(path)
            print(f"progress: path:{len(path)}", end='\r', flush=True)
            if node == "out":
                self.paths.add(pathStr)
            else:
                for neighbor in graph[node]:
                    if neighbor not in path:
                        arr = copy.deepcopy(path)
                        q.append((neighbor, arr))
        return path

    def getPaths(self, input: List[str]):
        graph, nodes, edges = self.parse(input)
        G = nx.Graph()
        G.add_nodes_from(nodes)
        G.add_edges_from(edges)

        # self.paths = nx.shortest_path(G, source="svr", target="out")
        self.paths = list(nx.all_simple_paths(G, source="svr", target="out"))

        print(self.paths)
        # return 0
        # print(graph)
        # self.paths = set()
        # self.dfs(graph, "svr", [])

        count = 0
        for path in self.paths:
            if "dac" in path and "fft" in path:
                count += 1
        # # print(self.paths)
        # print("")
        return count


obj = Solution()

input = [
    "svr: aaa bbb",
    "aaa: fft",
    "fft: ccc",
    "bbb: tty",
    "tty: ccc",
    "ccc: ddd eee",
    "ddd: hub",
    "hub: fff",
    "eee: dac",
    "dac: fff",
    "fff: ggg hhh",
    "ggg: out",
    "hhh: out",
]
expected = 2
result = obj.getPaths(input)
printResult(result, expected)

input = [
    "svy: npd tiy oux als",
    "clh: nlj",
    "qjt: efj hsv",
    "kwg: bqs tiq rir",
    "twr: lyq ibs",
    "kol: yil",
    "gzb: coh rsy",
    "fft: aya pvu",
    "gmj: fda",
    "rhn: xty you wkd ltb",
    "iyo: gxf vmy vsx lss",
    "olz: qlq",
    "ohd: tmu cea apx",
    "cmw: fsc zcz",
    "kow: vop jul",
    "pbx: lqf rxr",
    "ava: fau gqy kkj",
    "jto: yur hlx",
    "pkn: hrm ujq hir",
    "efj: oeb",
    "sgs: chu ajq",
    "qya: cvf vid",
    "vlw: nrp ilm tfo",
    "rxe: thu",
    "psq: djx xtd ygh",
    "obl: ttw kdb uef hbk",
    "ngc: tfs skn bcy tea qmb",
    "hat: skz lvr fmt xij",
    "egk: lau esm",
    "imr: lau cvo wwx esm",
    "oxy: jmn pbs",
    "txd: vzm uvc ljo",
    "tsv: gyz ibs lyq",
    "cpj: ahl lmq",
    "mla: pzo",
    "cvf: wom oxy",
    "faz: hqg val ybz",
    "qmk: pxp",
    "ptr: nvm hzi",
    "gvl: ico mpt syt kab tvw gqm gmj aeb ajw rdd nrr fte xpm gsg bap ioe kkm rup cqj",
    "pzo: ttq zpu sbi vlw dpf",
    "ffg: ljo vzm mbe",
    "chu: kzg kcn wze ikl mgs",
    "mkx: ibl cqj rup kkm syt lgf ajw tvw kab fte",
    "yfq: rxd iox dpe rkv",
    "mvn: out",
    "one: you xty",
    "gzl: jkz lmd",
    "vuh: xhv fuj uoq xgu",
    "bss: yno",
    "rkv: juo",
    "mqd: out",
    "vzm: qjt hgr xoz flp vpo yiq zzj toq xbn",
    "had: hjw",
    "ybz: ltb wkd you",
    "iox: mqd juo",
    "ltb: xee msy ewx boj owr oah rjx avv dzn gvs xuo jmy bvd",
    "sfs: mbe ljo",
    "tmu: qku ujy",
    "skn: wff",
    "agr: kwg klr tbk",
    "alq: obv pzd",
    "mle: ikn nhi pzo wxj",
    "ahl: gzo ike jwz knm",
    "vsx: lau",
    "rqc: rjf wfq wyx dfy ddd",
    "joh: bni rwy",
    "ikl: yur",
    "yno: glh iag dac",
    "yiq: cvf vid",
    "fyy: mma qdl",
    "wzr: wwx esm lau",
    "wxu: jhm",
    "jct: bjc als oux npd",
    "klr: tiq fyy",
    "iva: buq jbt bvp",
    "pvu: vqv mbe vzm uvc",
    "wwx: oeo ohd hxi vyk tiw awf bsz xqr",
    "cea: ujy rsj lot",
    "dzn: gte lvr fmt skz",
    "hrm: btf txd xly uxa",
    "knm: jto adq",
    "tiu: nhi",
    "oeo: rqc qlq gjw",
    "ajq: kcn kzg",
    "cjv: thu yur",
    "ugu: whf",
    "uxa: uvc vzm mbe ljo vqv",
    "xty: avv eiy wxu ccn dzn xuo gvs oah owr boj ewx msy xee avb",
    "avv: had iyh eoi",
    "oeb: esm wwx cvo",
    "ujq: xam btf txd uxa",
    "vqo: cuq ssj sfs",
    "ucb: fsp pno",
    "val: ltb xty",
    "qdl: nye uqk tpn",
    "xly: vqv vzm",
    "aml: hpw iyo muw yyr",
    "fda: mnf yil",
    "xbn: tou fxx efj",
    "cka: bgs oix ghq zmz",
    "ear: ilm nrp yno",
    "gte: ubh",
    "prp: lyq gyz",
    "ykt: doe jwp wrx",
    "bqk: whf",
    "rfu: ocm wzj",
    "vcz: rbg wnr svy jct",
    "hir: xly btf",
    "vno: wkd you",
    "zfd: jkz ieb",
    "psi: rxe",
    "wep: lky xvc",
    "tiq: mma qdl",
    "jbt: fjz hhx sqs",
    "wze: thu hlx yur",
    "jwz: jto adq",
    "rtz: xty wkd",
    "bwe: yur thu hlx",
    "ysw: skg hzc",
    "lss: cvo lau esm",
    "aeb: zfe ven rsy",
    "fmt: ubh zva rkf",
    "okm: ipn bar avg qyh",
    "zwa: klr kwg",
    "dac: wuv xgu xhv",
    "lzd: ieb jkz",
    "pbs: esm wwx lau cvo",
    "lpi: ahl han lmq",
    "cjl: yyr tzj hpw iyo",
    "lot: fib bno",
    "bqs: txz",
    "tga: oxy",
    "owr: lvr gte skz",
    "rgt: aml cjl",
    "wwu: yur hlx",
    "qku: bef",
    "hhx: wwm vdq siz",
    "qoc: vqo skg hzc",
    "aya: mbe vzm uvc",
    "ipn: hqg val",
    "ssf: dvu qmk lxw",
    "toa: lmd ieb",
    "qyh: ybz",
    "npd: vpc",
    "psm: nmd njl",
    "jes: cvo",
    "bsz: gjw zmx rqc",
    "zmz: orv bzm",
    "ibs: zlm ugu psm",
    "iyh: hjw yru ksu",
    "lqf: jes urs",
    "rdd: rbp ucb",
    "aaj: lmq han",
    "mjc: out",
    "uwe: qfc ewu pme",
    "eaa: cjl cpw",
    "uju: rgt eaa",
    "xuo: zwa",
    "kzg: yur hlx",
    "bdt: aml",
    "djx: iup cfp lmx gdr",
    "hlx: lzd toa aiw knp plo ava qoc agx zgk cka bsn",
    "mlw: qll",
    "wzj: egk gws rvl",
    "qmb: tsd",
    "ibl: xww",
    "ioe: svy wnr jct",
    "gxf: cvo lau",
    "gjw: wyx dfy rjf",
    "bjc: bwe cjv vpc",
    "cfp: rhn vdd",
    "jmn: cvo esm",
    "gqm: xww lay",
    "rjx: nkn clh",
    "flp: tga",
    "shn: ffg aya",
    "wff: vqv uvc vzm mbe",
    "xlx: rer",
    "kmw: iva",
    "ddd: ymr wau ojn",
    "fau: pkn ahb hcv nup",
    "xij: ubh",
    "tvw: ejp",
    "irc: you xty wkd ltb",
    "msy: had eoi",
    "ccn: agr zwa",
    "fib: wxj",
    "sus: zcz fsc clu",
    "als: cjv dje bwe",
    "wuv: one dgu rtz",
    "tzj: gxf vmy lyc vsx lss",
    "zlm: njl whf nmd",
    "jhw: vzm ljo vqv",
    "mor: rfu rer ysr",
    "xam: uvc vzm mbe ljo vqv",
    "uhp: apx",
    "fxx: oeb wzr",
    "nye: out",
    "bvp: ppi",
    "syt: coh rsy zfe ven",
    "gvs: jfo bcs fvt",
    "eck: out",
    "vhk: irc",
    "hqs: ykt",
    "fxj: njl whf",
    "ijr: btu ykt ray onr",
    "zup: dpe iox rkv rxd",
    "plo: vqo elj",
    "ttq: okm",
    "rwy: hlx thu",
    "azy: lgf ico syt kab tvw vcz gsg ibl gzb bap rup cqj aeb xpm nrr fte",
    "apy: rvl gws",
    "fuj: rtz one",
    "gqy: pkn ahb wqd hcv",
    "lyc: lau wwx",
    "adq: yur thu",
    "grj: wwx esm cvo",
    "zqt: bni",
    "buq: sqs mid",
    "gzo: vav jto",
    "yye: clh iva",
    "qlq: wfq wyx ddd",
    "ewx: agr",
    "wfq: ymr ojn eez",
    "egm: xam",
    "bap: kfj nnl jve",
    "nlj: mid hhx",
    "wzv: djx",
    "lmq: knm ike",
    "vav: hlx thu",
    "eet: xtd",
    "qkw: han",
    "wwm: mvn wbx mjc",
    "glh: xgu wuv uoq xhv",
    "fsp: ajq chu sxs",
    "svr: dcv ihi mkx azy gvl",
    "bar: hqg luj",
    "ujy: bef bno ifr",
    "pzd: pnx grj",
    "iqw: ocm",
    "xvc: qmb tea bcy skn",
    "xxd: clh nkn iva",
    "bef: nhi ikn",
    "kdb: ikn pzo wxj",
    "avb: fvt bcs jsg",
    "szw: xty ltb wkd",
    "hgr: vid",
    "btf: uvc vqv ljo",
    "qbs: han",
    "lvr: zva",
    "iup: gjg vdd",
    "prz: wqz vlc qgt vkv khf",
    "fjz: siz",
    "cvo: drk iwj prp dit awf",
    "vof: lau cvo esm wwx",
    "zfe: nut ywg",
    "emg: okk obl",
    "hoz: vqv mbe vzm uvc",
    "onr: doe",
    "eoi: ksu",
    "oux: vpc cjv",
    "fom: rwy rxe bni",
    "vss: wxj",
    "btu: jwp wrx doe",
    "jhm: tbk klr kwg",
    "lgf: kfj jve",
    "vqv: flp mor uld pbx qya mul vpo kvt bqv xoz toq uju",
    "doe: out",
    "rxm: mvn",
    "ozo: okm gco dul",
    "vkv: out",
    "hcv: hir ujq hrm",
    "pua: wcz alq mlw",
    "knp: siu fau",
    "qll: rly vrl grj",
    "rir: mma",
    "pbd: jmn pbs imr",
    "wcz: qll obv pzd",
    "cwa: huq obl okk",
    "siz: ntc mvn mjc",
    "vyk: sus twc cmw",
    "lxw: onq yam prz",
    "jul: ang",
    "muw: gxf lyc vmy vsx",
    "kab: jct svy wnr rbg",
    "mnf: psi hsq fom zqt joh",
    "iag: wuv fuj xhv",
    "rvl: wwx esm cvo",
    "ggp: mlw",
    "orv: vqv uvc vzm mbe",
    "boj: jsg fvt jfo",
    "fvc: wxj pzo ikn nhi",
    "ikn: wzv zpu ear ttq eet dpf ztc mjy ozo uwe agw psq kow bss xzs vgd vlw vnu",
    "fes: ljo mbe uvc vzm",
    "vdd: you xty wkd ltb",
    "qfc: hzi ere",
    "ahb: egm hrm",
    "agx: dty",
    "xhv: dgu rtz",
    "wli: qmk lxw tfx",
    "siu: hcv wqd",
    "ven: nut xrc wwu",
    "rjt: hsv",
    "xtd: gdr iup tkk cfp",
    "ygh: iup cfp gdr lmx",
    "esc: hrl ghq",
    "mul: iqw",
    "yru: hqs amc ijr",
    "yam: qgt vlc",
    "dfq: thu hlx",
    "ilm: dac glh iag vuh",
    "apx: ujy lot rsj",
    "onq: wqz",
    "hxi: apo cmw twc sus",
    "xqr: ibs",
    "lge: cxr jes",
    "reg: rxd",
    "dvi: dab",
    "njl: ikn nhi wxj",
    "zgk: siu fau kkj",
    "fvt: ssf",
    "ysr: apy xbl",
    "tiw: sus",
    "kkm: ucb rbp",
    "gws: cvo lau wwx",
    "rer: ocm apy",
    "dgu: xty you wkd ltb",
    "uld: tou",
    "ztc: vop",
    "vop: bly vhk",
    "jkz: fft shn opy",
    "ljo: toq zzj uju flp xlx dsn bqv rjt kvt",
    "wqd: ujq hir",
    "rup: qbs cpj qkw",
    "hsv: wzr vof",
    "gco: bar qyh",
    "awf: sus apo cmw twc",
    "zpu: jvz",
    "rbg: tiy npd als bjc",
    "apo: zcz clu fsc",
    "sjg: xvc lky",
    "kkj: ahb pkn",
    "mxx: lxw",
    "pme: hzi",
    "kwq: odg",
    "hpw: vmy",
    "skz: rkf",
    "dul: faz ipn bar",
    "bni: yur thu",
    "pnx: esm wwx lau",
    "dsn: lge lqf",
    "lau: bsz dit twr iwj prp vyk drk emg olz tiw tsv uhp cwa nmb ckp",
    "tbk: bqs fyy rir",
    "zmx: ddd wyx wfq rjf",
    "lmx: rhn",
    "hoc: out",
    "ghq: hoz qyk orv",
    "xuc: qmk dvu",
    "oix: hoz qyk bzm",
    "zcz: mle uhy omr",
    "lay: uep",
    "iwj: apx",
    "wqz: out",
    "mpt: cfl ucb rbp",
    "zue: rxd",
    "bly: irc szw",
    "wrx: out",
    "rsy: xrc wwu ywg nut",
    "ssj: ljo mbe vzm uvc",
    "pno: ajq",
    "gsg: qkw lpi",
    "mog: vqo",
    "tfx: prz pxp nub",
    "bcy: hiu",
    "xpm: ejp fda kol dvi",
    "skg: sfs trj",
    "bvd: had iyh uim eoi",
    "jvz: vhk ang",
    "rkf: zue tmg reg",
    "fte: qbs cpj qkw aaj lpi",
    "epz: yur",
    "dit: apo cmw twc sus",
    "thu: ivz zfd toa itr cka zgk ava gzl",
    "jmy: had iyh uim",
    "agw: tfo ilm",
    "ifr: ikn pzo",
    "yur: ivz knp plo aiw bsn cka itr zfd irl ysw mog hxy zgk esc",
    "ytk: apy xbl wzj",
    "vmy: lau cvo",
    "rbp: pno seh qxm",
    "yil: zqt psi",
    "mgs: yur thu",
    "oah: jsg",
    "avg: hqg ybz",
    "leu: lyq",
    "wkd: avv xuo kmw boj oah",
    "juo: out",
    "uhy: wxj ikn nhi",
    "wyx: ymr",
    "qxm: sxs chu ajq",
    "elj: yzu trj ssj cuq",
    "ico: nnl kfj",
    "jwp: out",
    "nub: qgt wqz khf",
    "dje: yur hlx",
    "wbx: out",
    "ppi: rxm",
    "vpc: yur hlx",
    "ray: jwp",
    "eez: bxs khd vss",
    "txz: tpn nye",
    "tkk: rhn",
    "hbk: pzo nhi",
    "tfo: dac iag glh vuh",
    "amc: ykt onr ray",
    "rjf: ojn",
    "dpf: dul",
    "xrc: yur thu",
    "hrl: hoz bzm orv qyk",
    "ike: jto vav",
    "xww: epz ztl uep dfq",
    "rxd: juo hoc",
    "yyr: lyc gxf lss vsx",
    "tsd: vqv ljo uvc",
    "hsq: rwy",
    "ojn: bxs khd",
    "jsg: xuc mxx",
    "nrr: kol fda bvf dvi",
    "tpn: out",
    "bob: nrp",
    "okk: kdb ttw",
    "dfy: eez ojn ymr",
    "tfs: tsd jhw wff fes",
    "nhi: eet ear zpu wzv ttq uwe hja mjy ztc sbi bob agw psq kow vlw vnu vgd xzs bss",
    "aiw: wep sjg dty",
    "lky: tea qmb skn",
    "trj: uvc vqv",
    "bsn: bgs oix zmz",
    "sqs: siz vdq wwm",
    "djo: pbs imr",
    "lyq: zlm ugu psm fxj",
    "dvu: pxp prz onq",
    "mma: uqk eck",
    "bwj: mlw alq",
    "mbe: mul qjt yiq clx xbn kvt bqv xoz hgr dsn xlx",
    "rsj: bno fib",
    "ajw: cfl",
    "ihi: aeb rdd ajw nrr xpm bap ioe gzb ibl vcz gsg cqj kkm rup syt ico gqm tvw kab",
    "han: nzn gzo ike jwz",
    "kve: gdr lmx",
    "wom: jmn",
    "bvf: dab mnf",
    "bno: wxj ikn pzo",
    "nnl: ztl",
    "uqk: out",
    "wxj: eet zpu ttq kow vnu uwe ozo mjy xzs bss ztc",
    "cfl: fsp sgs qxm seh pno",
    "luj: wkd xty",
    "tli: nvm",
    "nup: hir egm",
    "vrl: esm wwx",
    "dty: lky xvc ngc",
    "omr: ikn pzo",
    "bgs: bzm",
    "vgd: pme ewu ptr tli",
    "rxr: urs cxr",
    "hdc: odg wcz",
    "dpe: mqd",
    "esm: iwj ohd vyk tiw leu twr xqr dit oeo nmb hxi awf cwa",
    "wnr: npd bjc",
    "irl: lmd jkz ieb",
    "nvm: xty",
    "toq: lge lqf rxr",
    "lmd: shn opy",
    "uep: thu",
    "vnu: ptr qfc ewu pme",
    "tmg: rkv",
    "cxr: esm lau",
    "urs: wwx lau",
    "jve: uep",
    "zva: reg tmg",
    "hxy: siu",
    "whf: wxj nhi pzo",
    "bqv: pua kwq",
    "dcv: ibl gsg rup gmj ajw syt fte xpm gqm",
    "ksu: hqs ijr",
    "dab: joh zqt fom hsq psi",
    "uoq: one dgu",
    "mjy: djx kve ygh",
    "ocm: egk rvl",
    "wau: vss bxs",
    "xgu: dgu",
    "tea: jhw fes hiu",
    "ivz: sjg",
    "rly: cvo lau",
    "hzi: xty wkd",
    "eiy: lvr fmt xij",
    "khd: nhi ikn wxj",
    "huq: uef kdb tiu",
    "odg: pzd qll",
    "gyz: fxj bqk",
    "tou: wzr",
    "xzs: vop jul jvz",
    "xee: clh",
    "bzm: ljo uvc mbe",
    "hqg: you wkd",
    "uef: pzo ikn nhi wxj",
    "khf: out",
    "vpo: bwj ggp hdc",
    "cuq: vzm ljo",
    "coh: ywg xrc wwu",
    "nut: thu",
    "qgt: out",
    "cpw: hpw yyr",
    "nzn: adq vav",
    "zzj: eaa rgt bdt",
    "qyk: vzm mbe ljo",
    "gdr: rhn vdd gjg",
    "sxs: wze ikl",
    "clx: eaa bdt fay",
    "pxp: khf qgt wqz",
    "ewu: nvm ere hzi",
    "ywg: hlx yur",
    "seh: ajq sxs",
    "bcs: xuc wli",
    "xbl: gws",
    "ejp: yil",
    "ckp: zmx gjw",
    "fay: aml",
    "ere: you ltb",
    "opy: aya ffg",
    "ymr: bxs",
    "xoz: rfu ytk ysr",
    "nrp: vuh iag",
    "obv: vrl",
    "uim: yru",
    "twc: clu",
    "vlc: out",
    "ntc: out",
    "vid: pbd djo",
    "jfo: wli mxx",
    "mid: wwm rxm",
    "fsc: mla",
    "ubh: reg zue yfq zup",
    "hja: djx kve xtd",
    "you: xxd avb jmy ccn ewx owr rjx yye msy hat",
    "itr: fau siu kkj",
    "ttw: wxj ikn nhi",
    "sbi: tli ptr pme ewu",
    "nmd: nhi",
    "hiu: mbe vzm uvc ljo",
    "nmb: okk huq",
    "clu: fvc uhy",
    "nkn: buq jbt",
    "tiy: vpc bwe",
    "ztl: hlx",
    "kfj: dfq uep epz",
    "drk: cea apx",
    "kvt: pua kwq ggp bwj hdc",
    "gjg: xty you ltb",
    "uvc: flp pbx mor qjt qya mul zzj xlx dsn rjt toq yiq",
    "vdq: ntc",
    "yzu: vzm mbe vqv",
    "bxs: nhi ikn pzo wxj",
    "hzc: cuq ssj",
    "hjw: ijr amc hqs",
    "kcn: thu",
    "cqj: ucb cfl rbp",
    "ieb: opy shn fft",
    "ang: vno",
]
expected = 470
result = obj.getPaths(input)
printResult(result, expected)

