from typing import List
from myUtils.Utils import printResult

class Solution:

    def isDuplicated2(self, number: str):
        n = len(number)
        if n > 2:
            mid = n // 2
            for i in range(1, mid + 2):
                num = number[0:i] * (n // i)
                # print("i: %s num: %s number: %s" %(i, num, number))
                if number == num:
                    return True
        return False

    def isDuplicated(self, number: str):
        n = len(number)
        mid = n // 2
        return number[mid:] == number[:mid]

    def getInvalidFromRange(self, start:int, end:int)-> List[int]:
        res = []
        for num in range(start, end + 1, 1):
            # print(num)
            sNum = str(num)
            if self.isDuplicated2(sNum) or self.isDuplicated(sNum):
                res.append(num)
                # print(sNum)
        return res

    def getInvalidIDs(self, ids:str) -> int:
        listIds = ids.split(",")
        startEndList = [ item.split("-") for item in listIds ]
        # print(startEndList)
        res = []
        for sStart, sEnd in startEndList:
            start = int(sStart)
            end = int(sEnd)

            res.extend(self.getInvalidFromRange(start, end))

            # print(start)
            # print(end)
        return sum(res)


obj = Solution()

input = "11-22"
expected = 33
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "95-115"
expected = 210
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "998-1012"
expected = 2009
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "1188511880-1188511890"
expected = 1188511885
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "222220-222224"
expected = 222222
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "1698522-1698528"
expected = 0
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "38593856-38593862"
expected = 38593859
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "565653-565659"
expected = 565656
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "824824821-824824827"
expected = 824824824
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "2121212118-2121212124"
expected = 2121212121
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "11-22,95-115,998-1012,1188511880-1188511890,222220-222224,1698522-1698528,446443-446449,38593856-38593862,565653-565659,824824821-824824827,2121212118-2121212124"
expected = 4174379265
result = obj.getInvalidIDs(input)
printResult(result, expected)

input = "16100064-16192119,2117697596-2117933551,1-21,9999936269-10000072423,1770-2452,389429-427594,46633-66991,877764826-877930156,880869-991984,18943-26512,7216-9427,825-1162,581490-647864,2736-3909,39327886-39455605,430759-454012,1178-1741,219779-244138,77641-97923,1975994465-1976192503,3486612-3602532,277-378,418-690,74704280-74781349,3915-5717,665312-740273,69386294-69487574,2176846-2268755,26-45,372340114-372408052,7996502103-7996658803,7762107-7787125,48-64,4432420-4462711,130854-178173,87-115,244511-360206,69-86"
expected = 31898925685
result = obj.getInvalidIDs(input)
printResult(result, expected)

